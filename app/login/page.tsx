...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Patients must use the patient portal (ID + OTP)
      if (data.user?.role === "PATIENT") {
        setError("Patients must sign in using the Patient Portal with their National ID.");
        return;
      }
      ...
      <div className="flex-1 flex items-center justify-center bg-gray-50 px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="mt-3 text-center">
            <Link href="/patient-login" className="inline-flex items-center gap-1.5 text-xs text-green-700 font-semibold bg-green-50 border border-green-200 px-4 py-2 rounded-xl hover:bg-green-100 transition-colors">
              🪪 Patient? Sign in with National ID →
            </Link>
          </div>
        </div>
      </div>
